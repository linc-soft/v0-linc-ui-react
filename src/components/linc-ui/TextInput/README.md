...
TextInput 是一个功能强大的文本输入组件，支持验证、长度限制、多种标签样式和自定义插槽等功能。
...
1. **受控模式**：使用 `value` 属性时，组件进入受控模式，必须通过 `onChange` 更新值
2. **验证触发**：`lazyRules` 可以优化性能，避免频繁验证
3. **IME输入**：组件完美支持日文等输入法，IME组合过程中不会触发验证
